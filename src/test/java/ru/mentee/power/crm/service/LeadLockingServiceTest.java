...
/**
 * Тест конкурентного доступа: два потока обновляют один Lead. С pessimistic lock обновления
 * выполняются последовательно, оба успешны. С optimistic lock (без @Lock) вторая транзакция
 * получает OptimisticLockException.
 */
@SpringBootTest
@ActiveProfiles("test")
public class LeadLockingServiceTest {

  @Autowired private LeadLockingService lockingService;
  ...
  void pessimisticLock_twoThreadsUpdateSameLead_sequentialSuccess() throws Exception {
    ...
    Lead lead =
        leadService.addLead(
            "lock-pess@example.com", "Company", LeadStatus.NEW, address, "+79990000001");
    UUID leadId = lead.id();

    ExecutorService executor = Executors.newFixedThreadPool(2);
    List<Callable<Lead>> tasks =
        List.of(
  ...
  @Test
  void optimisticLock_twoThreadsUpdateSameLead_secondGetsOptimisticLockException()
      throws Exception {
    Address address = new Address("Moscow", "Street", "123456");
    Lead lead =
        leadService.addLead(
            "lock-opt@example.com", "Company", LeadStatus.NEW, address, "+79990000002");
    UUID leadId = lead.id();

    ExecutorService executor = Executors.newFixedThreadPool(2);
    Future<Lead> f1 =
        executor.submit(
            () -> lockingService.updateStatusWithOptimisticLock(leadId, LeadStatus.CONTACTED));
    Future<Lead> f2 =
        executor.submit(
            () -> lockingService.updateStatusWithOptimisticLock(leadId, LeadStatus.QUALIFIED));
    ...
    if (ex1 != null) {
      assertThat(ex1.getCause())
          .isInstanceOfAny(
              ObjectOptimisticLockingFailureException.class, StaleObjectStateException.class);
    }
    if (ex2 != null) {
  ...
  void convertWithPessimisticLock_twoThreadsSecondWaits() throws Exception {
    ...
    Lead lead =
        leadService.addLead(
            "convert-lock@example.com", "Company", LeadStatus.QUALIFIED, address, "+79990000003");
    UUID leadId = lead.id();

    ExecutorService executor = Executors.newFixedThreadPool(2);
    Future<ru.mentee.power.crm.domain.Deal> f1 =
        executor.submit(
