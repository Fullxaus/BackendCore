...
/**
 * Сервис для работы со статусами лидов. Добавление и получение статусов выполняется через этот
 * сервис.
 */
@Service
public class LeadStatusService {
  ...
  /**
   * Инициализирует хранилище всеми значениями enum LeadStatus, если оно пусто. Вызывать при старте
   * приложения (например, из DataInitializer или Main).
   */
