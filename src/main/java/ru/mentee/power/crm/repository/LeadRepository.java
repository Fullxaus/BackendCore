...
/**
 * JPA Repository для работы с LeadEntity. Расширен derived methods, JPQL запросами, пагинацией и
 * bulk операциями.
 */
public interface LeadRepository extends JpaRepository<LeadEntity, UUID> {
  ...
  /**
   * Поиск лидов по названию компании (поле company / companyName). SQL: SELECT * FROM leads WHERE
   * company = ?
   */
  ...
  /** Проверка существования по email. SQL: SELECT EXISTS(SELECT 1 FROM leads WHERE email = ?) */
  ...
  List<LeadEntity> findByStatusAndCompany(
      @Param("status") String status, @Param("company") String company);

  /**
   * Поиск с сортировкой по дате создания (от новых к старым). SQL: SELECT * FROM leads WHERE status
  ...
  /**
   * Поиск лида по ID с пессимистической блокировкой (SELECT ... FOR UPDATE). Используется для
   * критической операции конверсии Lead → Deal. Вторая транзакция ждёт завершения первой — lost
   * update не происходит.
   */
  ...
  /**
   * Поиск лидов по списку статусов (JPQL). JPQL: SELECT l FROM LeadEntity l WHERE l.status IN
   * :statuses SQL: SELECT * FROM leads WHERE status IN (?, ?, ...)
   */
  ...
  /**
   * Поиск лидов созданных после определённой даты. JPQL: SELECT l FROM LeadEntity l WHERE
   * l.createdAt > :date
   */
