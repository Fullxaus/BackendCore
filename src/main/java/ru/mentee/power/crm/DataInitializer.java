...
public class DataInitializer implements CommandLineRunner {
  ...
  public static final UUID QUALIFIED_LEAD_ID =
  ...
  public void run(String... args) {
    ...
    log.info(
        "Статусы лидов инициализированы: {} записей", leadStatusService.findAllStatuses().size());
    ...
    if (coreLeadRepository.findById(QUALIFIED_LEAD_ID) == null) {
      Lead qualifiedLead =
          new Lead(
              QUALIFIED_LEAD_ID,
              new Contact(
                  "ivan@example.com", "+79991234567", new Address("Moscow", "Lenina", "101000")),
              "Иван Петров",
              LeadStatus.QUALIFIED.name());
      coreLeadRepository.save(qualifiedLead);
    }

    // 10 Deals для Kanban (Scenario 4): 3 NEW, 2 QUALIFIED, 2 PROPOSAL_SENT, 1 NEGOTIATION, 1 WON,
    // 1 LOST
  ...
  /**
   * Заполняет все обязательные поля для существующих лидов, если они пустые или null. Это
   * исправляет проблему с данными, которые были созданы до добавления валидации.
   */
  private void fillMissingFieldsForAllLeads() {
    ...
    for (Lead lead : allLeads) {
      ...
      log.debug(
          "Проверка лида {}: email={}, phone={}, company={}, city={}, zip={}",
          lead.id(),
          email,
          phone,
          company,
      ...
      if (email == null
        ...
        while (existingLead.isPresent() && !existingLead.get().id().equals(lead.id())) {
          ...
          uniqueEmail =
              "lead" + lead.id().toString().replace("-", "") + "_" + counter + "@example.com";
      ...
      if (addressNeedsUpdate) {
        ...
        street =
            (street != null && !street.isEmpty() && !street.equals("-"))
                ? street
                : "Unknown Street";
        ...
        needsUpdate = true;
        log.debug(
            "Обновление address для лида {}: city={}, street={}, zip={}",
            lead.id(),
            city,
            street,
            zip);
      ...
      if (needsUpdate) {
        try {
          ...
          updatedCount++;
          log.info(
              "Обновлен лид {}: email={}, phone={}, company={}, city={}, zip={}",
              lead.id(),
              email,
              phone,
              company,
          ...
          log.error(
              "Текущие значения: email={}, phone={}, company={}, address={}",
              email,
              phone,
              company,
              address);
    ...
    log.info(
        "Заполнение обязательных полей завершено. Проверено лидов: {}, обновлено: {}",
        checkedCount,
        updatedCount);
  }
  ...
  private void addLeadIfNotExists(
  ...
  private void createDealsForKanban(java.util.List<UUID> leadIds) {
    ...
    var lead7 =
        addLeadIfNotExistsAndGet(
            "deal7@example.com", "DealCompany7", LeadStatus.QUALIFIED, addr, "+79991234567");
    if (lead7 != null) {
    ...
    var lead8 =
        addLeadIfNotExistsAndGet(
            "deal8@example.com", "DealCompany8", LeadStatus.QUALIFIED, addr, "+79991234568");
    if (lead8 != null) {
    ...
    var lead9 =
        addLeadIfNotExistsAndGet(
            "deal9@example.com", "DealCompany9", LeadStatus.QUALIFIED, addr, "+79991234569");
    if (lead9 != null) {
  ...
  private Lead addLeadIfNotExistsAndGet(
      String email, String company, LeadStatus status, Address address, String phone) {
    return leadService
        .findByEmail(email)
