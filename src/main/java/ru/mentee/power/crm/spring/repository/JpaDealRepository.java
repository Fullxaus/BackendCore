...
public class JpaDealRepository implements DealRepository {
  ...
  public List<Deal> findAll() {
    return entityRepository.findAll().stream().map(this::toModel).toList();
  }

  @Override
  public List<Deal> findByStatus(DealStatus status) {
    return entityRepository.findByStatus(status.name()).stream().map(this::toModel).toList();
  }
  ...
  private Deal toModel(DealEntity e) {
    LocalDateTime createdAt =
        LocalDateTime.ofInstant(
            e.getCreatedAt() != null ? e.getCreatedAt() : Instant.now(), ZoneId.systemDefault());
    return new Deal(
        e.getId(), e.getLeadId(), e.getAmount(), DealStatus.valueOf(e.getStatus()), createdAt);
  }
}
